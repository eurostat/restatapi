...
id<-"htec_cis3"
t1<-system.time({dt1<-get_eurostat_data(id,keep_flags=FALSE,cflags=TRUE)})[3]
nc1<-ncol(dt1)
t2<-system.time({dt2<-get_eurostat_data(id,stringsAsFactors=FALSE)})[3]
nc2<-ncol(dt2)
...
if (!is.null(xml_toc)){
  ...
  test_that("test of the get_eurostat_raw/bulk/data functions", {
    ...
    expect_message(dt3<-get_eurostat_data(id,verbose=FALSE))
...
test_that("test filtering in the get_eurostat_data function", {
  ...
  expect_message(dt8<-get_eurostat_data("avia_par_me",filters="HU",date_filter="2017-03",select_freq="Q",label=TRUE))
...
if (!is.null(dsd1)&is.data.frame(dsd1)){
  dt10<-get_eurostat_data("agr_r_milkpr",filters="AT")
  nc10<-ncol(dt10)
  nr10<-nrow(dt10)
  dt11<-get_eurostat_data("agr_r_milkpr",filters="AT",exact_match=FALSE,keep_flags=TRUE)
  nc11<-ncol(dt11)
  nr11<-nrow(dt11)
  ...
  nr12<-nrow(get_eurostat_data("agr_r_milkpr",date_filter=2016))
  nr13<-nrow(get_eurostat_data("agr_r_milkpr",date_filter="2016",keep_flags=TRUE))
  if (!is.null(nr12)&!is.null(nr13)){
    test_that("test filtering in the get_eurostat_data function", {
      expect_equal(nr12,nr13)
    })
  }
  nr14<-nrow(get_eurostat_data("agr_r_milkpr",filters="AT",exact_match=FALSE,keep_flags=TRUE,ignore.case=TRUE))
  if (!is.null(nr14)&!is.null(dt10)){
    test_that("test filtering in the get_eurostat_data function", {
      expect_true(nr14>nr10)
    })
  } 
  nr15<-nrow(get_eurostat_data("agr_r_milkpr",filters="BE$",date_filter="2006-02:2008-06-05",label=TRUE))
  if (!is.null(nr15)){
    test_that("test filtering in the get_eurostat_data function", {
      expect_equal(nr15,2)
    })
  }
  nr16<-nrow(get_eurostat_data("agr_r_milkpr",filters="BE",date_filter="<2008",cflags=TRUE))
  if (!is.null(nr16)){
    test_that("test filtering in the get_eurostat_data function", {
      expect_equal(nr16,11)
    })
  }  
  nr17<-nrow(get_eurostat_data("avia_par_me",filters="Q...ME_LYPG_HU_LHBP+ME_LYTV_UA_UKKK",date_filter=c("2017-07-01"),select_freq="M",cflags=TRUE))
  if (!is.null(nr17)){
    test_that("test filtering in the get_eurostat_data function", {
...
if (grepl("\\.amzn|-aws",Sys.info()['release'])) {
  ...
  if (!is.null(dsd2)){
    ...
    dt8<-get_eurostat_data("avia_par_me",filters="BE$",date_filter=c("2017-03",2016,"2017-07-01",2012:2014),select_freq="Q",label=TRUE,verbose=FALSE,name=FALSE)
    if (!is.null(dt8)){
